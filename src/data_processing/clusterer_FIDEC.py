...
class ClustererFIDEC(ClustererBase):
    ...
    def fit_predict(self,
        ...
        self.dec_dims = np.hstack((data.shape[-1], dec_dims, data_features.shape[-1]))

        fidec = DEC(dims=self.dec_dims, feature_dims=data_features[0].shape, n_clusters=num_clusters, init=init)
        fidec.pretrain(x=data, y=None, optimizer=pretrain_optimizer,
        epochs=pretrain_epochs, batch_size=batch_size,
        save_dir=save_dir)

        fidec.model.summary()
        fidec_input = [data, data_features]

        fidec.compile(optimizer=optimizer, loss=loss)
